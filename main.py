...
def elewait(): #defines a elewait function, this is the elevator slowly going down the several floors before arriving at one which will be defined by 'elefloor' which will lead to rooms which will then be defined by a 'eleroom' function
  ...
  roomstatus = random.choice(["has reached a floor", "hasn't reached a floor yet"]) #status of weather the elevator is on a floor or not, very important later.
  ...
  time.sleep(4) #waits 4 seconds before generating next 'elewaitmgen' but only if roomstatus is in the 'hasn't reached a floor yet' state.
init()
def elefloor(): #defines the 'floor' which wil be generated when you say 'y'
  ...
  luxcolfl = random.choice(["White", "Yellow", "Soft Yellow", "Dark Blue", "Soft Blue", "Dark Red", "Soft Red"]) ##the colfl's are needed to be seperate I swear!11!
  ...
  elefloorgen = f"you have stepped out of the elevator and are located in a {colfl} {matflwall} {flstructure}, the {flstructure} has a seies of 5 {luxbrightfl} {luxtype} lights. on the speakers {musicfl} is playing at a {volmusic2} volume."
  print (elefloorgen)
